...
async function main() {
  ...
  const allFiles = fs.readdirSync(datasetDir).filter((file: string) => {
  ...
  const existingFileNames = new Set((existingRecords || []).map((r: any) => r.file_name));
  const existingStoragePaths = new Set((existingRecords || []).map((r: any) => r.storage_path));
